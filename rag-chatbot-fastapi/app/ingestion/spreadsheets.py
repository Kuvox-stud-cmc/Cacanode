...
from app.ingestion.extraction import NormalizedColumn, NormalizedRow, NormalizedTable
...
def parquet_bytes(table: NormalizedTable) -> bytes:
    try:
        ...
        raise PermanentIngestionError("Parquet dependency polars is not installed") from exc
...
class PolarsCalculationAdapter:
    ...
    def execute(self, table: NormalizedTable, command: CalculationCommand) -> CalculationResult:
        if table.table_id != command.table_id:
            raise PermanentIngestionError("Calculation table selection is ambiguous or invalid")
        try:
            import polars as pl
        except ImportError as exc:
            raise PermanentIngestionError("Calculation dependency polars is not installed") from exc
        ...
        if unknown:
            raise PermanentIngestionError(f"Unknown spreadsheet column: {sorted(unknown)[0]}")
...
def _aggregation(expression: Any, operation: str) -> Any:
    ...
    raise PermanentIngestionError("Unsupported calculation operation")
