...
function spreadsheetFields(text: string): Array<[string, string]> {
  return text
    .split(";")
    .map((part) => part.trim())
    .map((part) => {
...
function SpreadsheetTable({
  ...
  const parsedRows = rows.map((unit) => ({
    unit,
    values: new Map(
      spreadsheetFields(unit.text).filter(
    ...
    >
      <div className="flex flex-wrap items-center gap-x-3 gap-y-2 border-b border-slate-200 bg-slate-50 px-4 py-3">
        <span className="inline-flex items-center gap-2 text-sm font-semibold text-slate-800">
          ...
          {metadataUnit.sheet_name ?? "Spreadsheet table"}
        </span>
        {summary?.cell_range && (
      ...
      <div>
        ...
        <div className="min-w-0 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
          <div className="flex items-center gap-2 border-b border-slate-200 bg-slate-50 px-4 py-3 text-xs font-semibold uppercase tracking-wider text-slate-500">
            <Table2 className="size-4 text-indigo-500" /> Indexed table
          </div>
          <div className="max-w-full overflow-x-auto overscroll-x-contain">
            <table className="min-w-max border-separate border-spacing-0 text-left text-xs sm:min-w-full sm:text-sm">
              <thead>
    ...
    return (
      <div>
        ...
        <dl className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
          {spreadsheetFields(unit.text).map(([label, value], index) => (
            <div
              key={`${label}-${index}`}
    ...
    <div>
      ...
      {blockType === "page" && (
        <div className="mb-3 flex items-center gap-2 text-xs font-semibold uppercase tracking-wider text-slate-400">
          <FileText className="size-3.5" /> Indexed page
        </div>
      )}
      {blockType === "sheet" && (
        <div className="mb-3 flex items-center gap-2 text-xs font-semibold uppercase tracking-wider text-slate-400">
          <Sheet className="size-3.5" /> Indexed sheet
        </div>
      )}
...
export default function DocumentViewerPage() {
  ...
  useEffect(() => {
    ...
    getDocumentUnitsApi(request, documentId)
      ...
      .catch((caught) => {
        if (!cancelled) setError(caught instanceof Error ? caught.message : "Unable to load indexed content");
      })
      .finally(() => {
  ...
  }, [document?.status, documentId, request]);
  ...
  async function downloadOriginal() {
    ...
    try {
      ...
      toast.error(caught instanceof Error ? caught.message : "Unable to download document");
    } finally {
  ...
  if (!document || error && document === null) {
    return (
      <Card><CardContent className="py-16 text-center"><p className="font-medium text-slate-800">Document unavailable</p><p className="mt-2 text-sm text-slate-500">{error}</p><Button variant="outline" className="mt-5" onClick={goBack}><ArrowLeft className="size-4" /> Back to documents</Button></CardContent></Card>
    );
  }
  ...
  return (
    <div className="mx-auto min-w-0 max-w-5xl space-y-4 sm:space-y-5">
      <div className="grid min-w-0 gap-3 sm:grid-cols-[auto_minmax(0,1fr)_auto] sm:items-center">
        <Button variant="ghost" size="sm" className="w-fit" onClick={goBack}><ArrowLeft className="size-4" /> Back</Button>
        ...
        <div className="flex min-w-0 items-center gap-2 sm:justify-end">
          <Badge variant="outline" className="shrink-0">{document.status === "COMPLETED" ? "Ready" : document.status === "PROCESSING" ? "Indexing" : document.status === "PENDING" ? "Pending" : "Failed"}</Badge>
      ...
      {evidenceUnits.length > 0 && (
        <div className="sticky top-[4.25rem] z-10 flex items-center justify-between gap-1 rounded-xl border border-indigo-200 bg-indigo-50/95 p-1.5 shadow-sm backdrop-blur sm:gap-3 sm:px-3 sm:py-2">
          <Button variant="ghost" size="sm" className="px-2 sm:px-3" disabled={selectedEvidence === 0} onClick={() => setEvidenceSelection({ focusKey, index: Math.max(0, selectedEvidence - 1) })}><ChevronLeft className="size-4" /><span className="hidden sm:inline">Previous</span></Button>
          <p className="whitespace-nowrap text-xs font-semibold text-indigo-900 sm:text-sm">Evidence {selectedEvidence + 1} of {evidenceUnits.length}</p>
          <Button variant="ghost" size="sm" className="px-2 sm:px-3" disabled={selectedEvidence === evidenceUnits.length - 1} onClick={() => setEvidenceSelection({ focusKey, index: Math.min(evidenceUnits.length - 1, selectedEvidence + 1) })}><span className="hidden sm:inline">Next</span><ChevronRight className="size-4" /></Button>
        </div>
      )}

      {document.status === "PENDING" || document.status === "PROCESSING" ? (
        <Card><CardContent className="flex min-h-[22rem] flex-col items-center justify-center text-center"><Loader2 className="mb-4 size-8 animate-spin text-indigo-600" /><h3 className="font-semibold text-slate-900">{document.status === "PENDING" ? "Waiting to index" : "Indexing document"}</h3><p className="mt-2 max-w-md text-sm leading-6 text-slate-500">The indexed viewer will appear here as soon as the document is ready.</p></CardContent></Card>
      ) : document.status === "FAILED" ? (
        <Card><CardContent className="flex min-h-[22rem] flex-col items-center justify-center text-center"><FileText className="mb-4 size-9 text-red-400" /><h3 className="font-semibold text-slate-900">Indexing failed</h3><p className="mt-2 max-w-md text-sm leading-6 text-slate-500">{document.errorMessage ?? "This document could not be indexed. You can still download the original file."}</p></CardContent></Card>
      ) : !unitsLoaded ? (
