...
function channelLabel(channel: CustomerConversation["channel"]): string { return channel === "WIDGET" ? "Widget" : "API"; }
...
export default function ConversationsPage() {
  ...
  useEffect(() => {
    ...
    listCustomerConversations(request, {
      ...
      if (!(error instanceof DOMException && error.name === "AbortError")) toast.error(error instanceof Error ? error.message : "Unable to load conversations");
    });
    return () => controller.abort();
  }, [channel, direction, page, request, requestKey, size, sort, startedFrom, startedTo, status, update, urlQuery]);
  ...
  async function closeConversation() {
    ...
    } catch (error) { toast.error(error instanceof Error ? error.message : "Unable to close conversation"); }
  }
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      <div className="flex flex-col gap-1 sm:flex-row sm:items-end sm:justify-between">
        ...
        <p className="text-sm text-slate-500">Widget and custom API channels only</p>
      </div>

      <div className="grid w-full grid-cols-3 gap-1 rounded-lg bg-slate-200 p-1 sm:w-fit">
        {filterTabs.map((value) => <button key={value} type="button" onClick={() => update({ status: value === "all" ? null : value })} className={cn("min-h-9 rounded-md px-3 text-sm font-medium", status === value ? "bg-white text-slate-900 shadow-sm" : "text-slate-600 hover:text-slate-900")}>{value === "all" ? "All" : value === "OPEN" ? "Open" : "Closed"}</button>)}
      </div>

      <FilterPanel>
        <UrlSearchField key={urlQuery} initialValue={urlQuery} onDebouncedChange={(value) => update({ q: value || null })} placeholder="Search customers or transcripts" />
        <FilterSelect label="Channel" value={channel} onChange={(value) => update({ channel: value === "all" ? null : value })}><option value="all">All channels</option><option value="WIDGET">Widget</option><option value="CUSTOM_API">Custom API</option></FilterSelect>
        <DateRangeFields prefix="Started" from={startedFrom} to={startedTo} onFromChange={(value) => update({ from: value || null })} onToChange={(value) => update({ to: value || null })} />
        <FilterSelect label="Sort" value={sortValue} onChange={(value) => update({ sort: value === "started-desc" ? null : value })}><option value="started-desc">Newest started</option><option value="started-asc">Oldest started</option><option value="activity-desc">Recent activity</option><option value="customer-asc">Customer A–Z</option></FilterSelect>
        <ClearFiltersButton disabled={!hasFilters} onClick={clear} />
      </FilterPanel>
      <ResultsSummary total={total} refreshing={refreshing} />

      <Card className="overflow-hidden">
        {!loaded ? <div className="grid min-h-56 place-items-center"><Loader2 className="animate-spin text-indigo-600" /></div> : items.length === 0 ? <div className="px-4 py-16 text-center"><MessageSquare className="mx-auto mb-3 size-9 text-slate-300" /><p className="text-sm font-medium text-slate-500">{hasFilters ? "No conversations match these filters" : "No customer conversations yet"}</p></div> : <>
          <div className="divide-y divide-slate-200 lg:hidden">{items.map((item) => <button key={item.id} type="button" onClick={() => void openConversation(item.id)} className="flex w-full items-start gap-3 p-4 text-left hover:bg-slate-50"><span className="grid size-10 shrink-0 place-items-center rounded-full bg-indigo-50 text-indigo-600"><MessageSquare className="size-4" /></span><span className="min-w-0 flex-1"><span className="flex justify-between gap-2"><span className="min-w-0"><span className="block truncate text-sm font-semibold">{customerLabel(item)}</span><span className="block truncate text-xs text-slate-500">{item.customer_email}</span></span><Badge variant="outline" className="shrink-0 text-[10px]">{item.status}</Badge></span><span className="mt-2 flex flex-wrap gap-2 text-xs text-slate-500">{channelLabel(item.channel)} · {item.message_count} messages · {formatDate(item.created_at)}</span></span><ChevronRight className="mt-2 size-4 text-slate-400" /></button>)}</div>
          <div className="hidden lg:block"><Table><TableHeader><TableRow><TableHead>Customer</TableHead><TableHead>Channel</TableHead><TableHead>Messages</TableHead><TableHead>Started</TableHead><TableHead>Status</TableHead><TableHead className="w-10" /></TableRow></TableHeader><TableBody>{items.map((item) => <TableRow key={item.id} className="cursor-pointer" tabIndex={0} onClick={() => void openConversation(item.id)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); void openConversation(item.id); } }}><TableCell><p className="font-medium">{customerLabel(item)}</p><p className="max-w-64 truncate text-xs text-slate-500">{item.customer_email}</p></TableCell><TableCell><Badge variant="outline">{channelLabel(item.channel)}</Badge></TableCell><TableCell>{item.message_count}</TableCell><TableCell>{formatDate(item.created_at)}</TableCell><TableCell><Badge variant="outline">{item.status}</Badge></TableCell><TableCell><Eye className="size-4 text-slate-400" /></TableCell></TableRow>)}</TableBody></Table></div>
        </>}
      ...
      <Dialog open={Boolean(selected) || detailLoading} onOpenChange={(open) => { if (!open) setSelected(null); }}><DialogContent className="flex max-h-[calc(100dvh-1rem)] w-[calc(100%-1rem)] max-w-2xl flex-col gap-0 overflow-hidden p-0"><DialogHeader className="border-b px-4 py-3 pr-12"><DialogTitle>Conversation</DialogTitle></DialogHeader>{detailLoading && !selected ? <div className="grid min-h-52 place-items-center"><Loader2 className="animate-spin" /></div> : selected && <div className="flex min-h-0 flex-1 flex-col p-3 sm:p-5"><div className="mb-3 flex flex-wrap items-center gap-2"><Badge variant="outline">{channelLabel(selected.channel)}</Badge><Badge variant="outline">{selected.status}</Badge><span className="break-all text-xs text-slate-500">{selected.customer_email || selected.external_user_id}</span></div><div className="min-h-0 flex-1 space-y-2 overflow-y-auto rounded-lg bg-slate-50 p-3">{selected.messages.map((message) => <div key={message.sequence_number} className={cn("flex", message.role === "user" ? "justify-end" : "justify-start")}><div className={cn("max-w-[90%] whitespace-pre-wrap break-words rounded-xl px-3 py-2 text-sm leading-6", message.role === "user" ? "rounded-br-sm bg-indigo-600 text-white" : "rounded-bl-sm border bg-white text-slate-700")}>{message.content}</div></div>)}</div>{selected.status === "OPEN" && <Button className="mt-3 w-full sm:w-fit" onClick={() => void closeConversation()}>Close conversation</Button>}</div>}</DialogContent></Dialog>
    </div>
  );
}
