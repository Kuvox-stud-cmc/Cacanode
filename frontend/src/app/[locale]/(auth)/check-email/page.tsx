...
import { authApiErrorMessage, resendVerificationApi } from "@/lib/auth-api";
import {
...
function CheckEmailContent() {
  ...
  const handleResend = useCallback(async () => {
    ...
    try {
      ...
      } else if (errorMsg.includes("wait") || errorMsg.includes("cooldown")) {
        setMessage({ type: "error", text: errorMsg });
      } else {
        setMessage({ type: "error", text: errorMsg });
      }
      ...
      <div className="w-full max-w-md">
        ...
        <Card className="shadow-md bg-white">
          <CardHeader className="text-center">
            ...
            <CardDescription>{t("checkEmail.sentTo")}</CardDescription>
          </CardHeader>
          <CardContent className="space-y-6">
            ...
            <div className="bg-slate-50 rounded-lg p-4 text-sm text-slate-600">
              <p className="mb-2">{t("checkEmail.instructions")}</p>
