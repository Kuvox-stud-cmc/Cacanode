...
export function validateTemplate(locale: "vi-VN" | "en-US", raw: RevisionContent, maximumDurationSeconds = 600): string | null {
  ...
  if (content.durationLimitSeconds > maximumDurationSeconds) return "templateDurationLimit";
...
export function TemplateForm({ templateId }: { templateId?: string }) {
  ...
  const [maximumDurationSeconds,setMaximumDurationSeconds]=useState(600);

  useEffect(()=>{const controller=new AbortController();getRecruitmentCapabilities(request,controller.signal)
    .then(value=>setMaximumDurationSeconds(value.maxInterviewDurationSeconds)).catch(()=>setMaximumDurationSeconds(600));
    return()=>controller.abort();},[request]);
  ...
  const clearLocaleDraft = useLocaleChangeDraft(
    `recruitment:template:${tenantId ?? "unknown"}:${templateId ?? "new"}`,
    { template, viewingRevisionId, editingContent },
    (draft) => {
      setTemplate(draft.template);
      setViewingRevisionId(draft.viewingRevisionId);
      setEditingContent(draft.editingContent);
    },
    Boolean(tenantId) && !loading && !record?.archived,
  );
  ...
  async function save() {
    const validation = validateTemplate(template.locale, template.content, maximumDurationSeconds);
    if (!template.name.trim()) { setError(t("forms.errors.templateName")); return; }
    if (validation) { setError(validation === "templateDurationLimit" ? t("durationLimits.error", { max: maximumDurationSeconds }) : t(`forms.errors.${validation}` as Parameters<typeof t>[0])); return; }
  ...
  async function archive() {
    if (!record || !await confirm({ title: `${t("forms.archiveTemplate")}: ${record.name}`, description: t("forms.confirm.archiveTemplate"), confirmLabel: t("actions.archive"), destructive: true })) return;
    setSaving(true); setError("");
    try { await archiveRecruitmentTemplate(request, record.id); clearLocaleDraft(); await load(); router.refresh(); }
  ...
  return <div className="mx-auto max-w-5xl space-y-4">
    ...
    <Card><CardHeader><CardTitle>{t("forms.spokenText")}</CardTitle><CardDescription>{readOnly ? t("forms.readOnlyRevision") : t("forms.spokenTextHelp")}</CardDescription></CardHeader><CardContent className="space-y-4">
      ...
      <div className="grid gap-4 md:grid-cols-3"><NumberField id="total-duration" label={t("forms.totalDuration")} min={1} max={maximumDurationSeconds} disabled={readOnly} value={template.content.durationLimitSeconds} onChange={(value) => setContent({ ...template.content, durationLimitSeconds: value })} /><NumberField id="repetition" label={t("forms.repetitionLimit")} disabled={readOnly} value={template.content.interactionLimits.repetitionLimit} onChange={(value) => setContent({ ...template.content, interactionLimits: { ...template.content.interactionLimits, repetitionLimit: value } })} /><NumberField id="clarification" label={t("forms.clarificationLimit")} disabled={readOnly} value={template.content.interactionLimits.clarificationLimit} onChange={(value) => setContent({ ...template.content, interactionLimits: { ...template.content.interactionLimits, clarificationLimit: value } })} /><NumberField id="silence-timeout" label={t("forms.silenceTimeout")} min={1} disabled={readOnly} value={template.content.interactionLimits.silenceTimeoutSeconds} onChange={(value) => setContent({ ...template.content, interactionLimits: { ...template.content.interactionLimits, silenceTimeoutSeconds: value } })} /><NumberField id="silence-prompts" label={t("forms.silencePrompts")} disabled={readOnly} value={template.content.interactionLimits.silencePromptLimit} onChange={(value) => setContent({ ...template.content, interactionLimits: { ...template.content.interactionLimits, silencePromptLimit: value } })} /></div><p className="text-xs text-muted-foreground">{t("durationLimits.help", { max: maximumDurationSeconds })}</p>
    </CardContent></Card>
    ...
    <div className="space-y-4">{template.content.sections.map((section, sectionIndex) => <Card key={section.sectionId}><CardHeader><div className="flex flex-wrap items-center justify-between gap-2"><CardTitle>{t("forms.sectionNumber", { number: sectionIndex + 1 })}</CardTitle>{!readOnly && <div className="flex gap-1"><Button size="icon-sm" variant="ghost" aria-label={t("forms.moveUp")} disabled={sectionIndex === 0} onClick={() => moveSection(sectionIndex, -1)}><ArrowUp /></Button><Button size="icon-sm" variant="ghost" aria-label={t("forms.moveDown")} disabled={sectionIndex === template.content.sections.length - 1} onClick={() => moveSection(sectionIndex, 1)}><ArrowDown /></Button><Button size="icon-sm" variant="ghost" aria-label={t("forms.removeSection")} disabled={template.content.sections.length === 1} onClick={() => setContent({ ...template.content, sections: template.content.sections.filter((_, index) => index !== sectionIndex) })}><Trash2 /></Button></div>}</div></CardHeader><CardContent className="space-y-4">
        <div className="grid gap-4 md:grid-cols-3"><div className="space-y-2"><Label htmlFor={`kind-${section.sectionId}`}>{t("forms.sectionKind")}</Label><select id={`kind-${section.sectionId}`} disabled={readOnly || template.locale === "en-US"} className="h-9 w-full rounded-lg border border-input bg-background px-3 text-sm disabled:opacity-60" value={section.kind} onChange={(event) => { const kind = event.target.value; updateSection(sectionIndex, { ...section, kind, languageTag: kind === "ENGLISH_SCREEN" ? "en-US" : template.locale }); }}><option value="CORE">{formatEnumLabel("CORE", uiLocale)}</option>{template.locale === "vi-VN" && <option value="ENGLISH_SCREEN">{formatEnumLabel("ENGLISH_SCREEN", uiLocale)}</option>}</select></div><div className="space-y-2"><Label htmlFor={`section-language-${section.sectionId}`}>{t("fields.language")}</Label><Input id={`section-language-${section.sectionId}`} disabled value={section.languageTag === "vi-VN" ? t("forms.viVN") : t("forms.enUS")} /></div><NumberField id={`section-duration-${section.sectionId}`} label={t("forms.sectionDuration")} min={1} max={section.kind === "ENGLISH_SCREEN" ? Math.min(300,maximumDurationSeconds) : maximumDurationSeconds} disabled={readOnly} value={section.durationLimitSeconds} onChange={(value) => updateSection(sectionIndex, { ...section, durationLimitSeconds: value })} /></div>
    ...
    {!record?.archived && (!record || editingContent) && <div className="flex justify-end gap-2"><Button variant="outline" nativeButton={false} render={<Link href="/recruitment/templates" onClick={clearLocaleDraft} />}>{t("actions.cancel")}</Button><Button disabled={saving} onClick={() => void save()}>{saving ? t("actions.saving") : record ? t("forms.saveRevision") : t("save")}</Button></div>}
