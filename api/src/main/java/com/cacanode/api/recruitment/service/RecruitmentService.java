...
public class RecruitmentService implements RecruitmentApplicationCommandApi, RecruitmentInterviewCommandApi {
    ...
    public CreatedApplication create(CreateApplicationCommand command){capabilities.requireMasterEnabled(command.tenantId());RecruitmentJob job=lockJob(command.tenantId(),command.jobId());if(job.getStatus()!=JobStatus.PUBLISHED&&job.getStatus()!=JobStatus.PAUSED)throw conflict("Applications require a published job");requireCandidate(command.tenantId(),command.candidateId());InterviewTemplateRevision revision=validateRevision(command.tenantId(),job.getTemplateRevisionId(),false);RecruitmentApplication a=new RecruitmentApplication();a.setTenantId(command.tenantId());a.setJobId(job.getId());a.setCandidateId(command.candidateId());a.setStatus(ApplicationStatus.AWAITING_CANDIDATE);a.setSubmittedAt(null);a.setLocale(job.getLanguage());a.setPrivacyConsentAt(null);a.setCvPresent(false);a.setCvAnalysisStatus(CvAnalysisStatus.NOT_REQUESTED);a.setCvAiModeSnapshot(job.getEffectiveCvAiMode()==null?CvAiMode.OFF:job.getEffectiveCvAiMode());a.setTemplateRevisionId(revision.getId());a.setTemplateSnapshot(revision.getContent());a.setTemplateSnapshotSha256(revision.getContentSha256());a.setTemplateSnapshotVersion(Integer.toString(revision.getRevisionNumber()));a.setScreeningConfigSnapshot(job.getScreeningConfig());a.setScreeningAnswers("[]");a.setAutomationModeSnapshot(job.getEffectiveAutomationMode()==null?AutomationMode.MANUAL:job.getEffectiveAutomationMode());try{a=applicationRepository.saveAndFlush(a);}catch(DataIntegrityViolationException e){throw conflict("Candidate has already applied to this job");}return new CreatedApplication(a.getId(),a.getStatus().name(),a.getTemplateSnapshotSha256());}
